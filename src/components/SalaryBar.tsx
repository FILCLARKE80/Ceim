  ...
  return (
    <div className="mt-3">
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="text-xs text-slate-400 dark:text-graphite">
          {euro(band.min)}–{euro(band.max)}
        </span>
      </div>

      {/* scale: 0 — 400% of the national average */}
      <div className="relative mt-2 h-2.5 w-full rounded-full bg-slate-100 dark:bg-white/10">
      ...
      <div className="relative mt-1 h-3.5 text-[10px] text-slate-400 dark:text-graphite">
        <span className="absolute left-0">0%</span>
        <span
          className="absolute -translate-x-1/2 font-medium text-slate-500 dark:text-slate-300"
          style={{ left: `${avgMarker}%` }}
        >
